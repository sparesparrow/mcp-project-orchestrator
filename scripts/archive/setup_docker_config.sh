...
echo "1. Rebuild the container: docker build -t mcp-project-orchestrator:latest -f Containerfile ."
echo "2. Test it directly: docker run --rm -v \"${PROJECT_DIR}:/app\" --workdir /app -p 8080:8080 --entrypoint python mcp-project-orchestrator:latest -m mcp_project_orchestrator.fastmcp"
