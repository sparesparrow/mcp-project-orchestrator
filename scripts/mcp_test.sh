...
# Run the container
echo -e "\n${COLOR_GREEN}Starting MCP server...${COLOR_RESET}"
if [ "$CONTAINER_ENGINE" == "docker" ]; then
    $CONTAINER_ENGINE run -d --rm -p 8080:8080 --name mcp-server mcp-project-orchestrator:latest
else
    $CONTAINER_ENGINE run -d --rm -p 8080:8080 --name mcp-server localhost/mcp-project-orchestrator:latest
fi
...
echo -e "  $CONTAINER_ENGINE run -d --rm -p 8080:8080 --name mcp-server mcp-project-orchestrator:latest"
